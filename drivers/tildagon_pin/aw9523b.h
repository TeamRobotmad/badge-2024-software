...
struct aw9523b_device;
/* Context is derived from (dev, pin) rather than stored per pin, so the table below
 * costs one pointer per pin instead of two. */
typedef void (*aw9523b_irq_callback_t)(struct aw9523b_device *dev, aw9523b_pin_t pin, uint8_t event);
...
typedef struct aw9523b_device
{
  ...
  aw9523b_irq_callback_t irq_handlers[2][8];
} aw9523b_device_t;
